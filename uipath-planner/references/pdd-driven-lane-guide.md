...
> Lane A is reached from the entry guard when the input file contains the marker heading `## Planner Handoff` (the load-bearing detection contract from `uipath-design`). See SKILL.md for the entry guard logic.
...
| **Tasks file** | Where to write `<process>-tasks.md`. uipath-design proposes a path; planner respects it. |
| **Generated by** | `uipath-design v<VERSION>`. Used in the summary output to tell the user the SDD's provenance. |
