...
Resolved by triage. Full paths to every playbook that matches the symptoms, with confidence level (high/medium/low). High-confidence playbooks have a specific error match and known cause. Medium have concrete troubleshooting steps. Low provide general context. The generator uses confidence to decide how many hypotheses to produce per playbook.
