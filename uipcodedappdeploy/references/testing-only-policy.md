...
- A `published-recovery` attempt must consume one exact schema-1.1 or schema-1.2
  ...
  claim, rejects a schema-1.2 receipt that already contains recovery evidence,
  and revalidates all bound bytes before invoking even the read-only guarded
  runtime. Its execution path contains no pack or publish operation.
