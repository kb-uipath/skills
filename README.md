...
| [cs-agentification-weekly-update](./cs-agentification-weekly-update/SKILL.md) | Extracts local Tribble Scribe call recordings through a read-only snapshot and drafts audience-calibrated Slack updates. | A weekly or ad hoc CS Agentification program update is needed for the exec and leads Slack channels, without sending directly. | [docs](./docs/cs-agentification-weekly-update.md) |
| [enrich-day2-dashboard](./enrich-day2-dashboard/SKILL.md) | Builds schema `1.4` Day 2 JSON through exact-safe Salesforce seeding, scoped evidence, bounded clarification, strict exact approval, or opt-in maximum evidence-backed draft coverage. | An account team needs executive-ready JSON with the fullest defensible content while keeping unsupported facts explicit. | [docs](./docs/enrich-day2-dashboard.md) |
...
| [salesforce-account-profile](./salesforce-account-profile/SKILL.md) | Builds a confidential read-only Account profile through a resumable conversation, exact selection, bounded family approval, and deterministic rendering. | An explicitly invoked Salesforce pipeline, team, family-map, or full selected-account profile is needed without user-managed transport, fuzzy auto-selection, caching, writes, or uncertified annualization. | [docs](./docs/salesforce-account-profile.md) |
...
| [uipcodedappdeploy](./uipcodedappdeploy/SKILL.md) | Generates source/dist/package/CLI-bound deployment plans and redacted resumable receipts before any exact-hash-approved UiPath deployment. | A coded app needs validated versioning, deterministic packaging, publication, deployment, and route verification with no default writes. | [docs](./docs/uipcodedappdeploy.md) |
