...
| 2 | `uipath-agentic-expansion-planner` | 6.5 | 8.9 | +2.4 | Inventory profiler tests, golden profile fixture, DOCX render/verify coverage, Markdown executive-brief quality validator, UiPath brand-style DOCX verification, brand-aware renderer colors/fonts, current metadata, and DOCX/fallback docs. | Live customer brief quality still depends on source inventory richness, public evidence validation, and approved customer-specific brand assets when required. |
